...
int main()
{
	...
	for(i=0;i<n;++i)
	{
		printf("Process ID : %d. Enter Arrival Time, Priority and Burst Times\n", i);
		pr[i].pid = i;
		pr[i].v = 1;
		scanf("%d%d%d", &pr[i].arrTime, &pr[i].priority, &pr[i].burstTime);
	}
...
/*
	...
	{
		j = i + 1;
		while(pr[i].arrTime == pr[j].arrTime)
		{
			++j;
		}
		sort_arr(pr, i, j);
